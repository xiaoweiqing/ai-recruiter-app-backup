...
def setup_local_database():
    ...
    try:
        with sqlite3.connect(LOCAL_DATABASE_FILE) as conn:
            ...
            # --- This is your existing 'reports' table ---
            ...
            # This is for the "Ultimate PK Showdown" results
            print("   -> [DB] Ensuring 'comparisons' table exists for Ultimate PK...")
            cursor.execute(
                """
            ...
            # --- This is your existing 'pk_reports' table for personal PKs ---
...
# FILE: database.py (add to the end of the file)


async def background_comparison_storage_task(task_data: dict, loop):
    """【后台任务】将 Ultimate PK (对比分析) 结果存入专属的 SQLite 表。"""
    try:
        ...
        await loop.run_in_executor(None, _sync_sqlite_write)
        print(f" > [Sync] Ultimate PK '{task_data['task_name']}' saved to SQLite.")
...
async def vectorize_and_store_comparison_async(task_data: dict, loop):
    """【后台任务】将完整的 Ultimate PK 报告向量化并存入 Qdrant。"""
    if not QDRANT_CLIENT or not EMBEDDING_MODEL:
        return

    full_report_text = f"对比任务: {task_data['task_name']}\n\nPK报告:\n{task_data.get('pk_report', '')}"
    try:
        vector = await loop.run_in_executor(
        ...
        await loop.run_in_executor(None, upsert_op)
        print(
            f" > [Qdrant Sync] Ultimate PK report for '{task_data['task_name']}' vectorized."
        )
    except Exception as e:
        print(
